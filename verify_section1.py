...
s, ps, _ = req("GET", "/tasks/project-stats")
check("GET /tasks/project-stats -> 200", s == 200, f"got {s}")
check("project-stats returns a list", isinstance(ps, list), str(type(ps)))
check("project-stats list non-empty", len(ps) > 0, str(ps))
if ps:
    first = ps[0]
    check("project-stats has 'project_id'",   "project_id"   in first, str(first))
    check("project-stats has 'project_name'", "project_name" in first, str(first))
    check("project-stats has 'total'",        "total"        in first, str(first))
    check("project-stats has 'completed'",    "completed"    in first, str(first))
    check("project-stats has 'pending'",      "pending"      in first, str(first))
# Verify the test project appears with correct total
matching = [p for p in ps if p.get("project_id") == project_id]
check("project-stats includes test project", len(matching) > 0, f"project_id={project_id}")
if matching:
    check("project-stats test project total > 0",
          matching[0].get("total", 0) > 0, str(matching[0]))
